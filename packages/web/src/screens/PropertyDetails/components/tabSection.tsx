...
const TabSections = (propsData: IProps): React.ReactElement => {
  ...
  const {
    assetDetails: { description, features, leaseTerm, saleTerm, amenityGroup, assetHighlights },
    propertyTermId,
  } = propsData;
  ...
  const renderTabScene = (route: IRoutes): React.ReactElement | null => {
    switch (route.key) {
      ...
      case Tabs.AMENITIES:
        return <Amenitites amenityGroup={amenityGroup} assetHighlights={assetHighlights} />;
